...
Since our target audience are the NUS students, which include a large proportion of international students, our Contact Book will support the users to add multiple phone numbers.

==== Model

UniquePhoneList Classes are created for each person to store the person's multiple phone numbers.
...
Primary number is saved directly as an object in Person while additional numbers are save in a list.
...
==== Logic
PhoneCommand is created to add/remove additional phone numbers. It can also be used to show all numbers of a person.

PhoneCommand is called by using command word "updatePhone", then the command word is followed by 3 parameters: Index, String, String(optional).
The 3 parameters represents the index of the person in the Contact Book, action to be taken (add/remove/showAllPhones) and a value respectively.

For add, we will add the phone to the uniquePhoneList and check for duplicate. Example call of add is illustrated as below.
image::add.png[width="200"]


For remove, we will loop through the uniquePhoneList and remove this phone and check for NoPhoneFoundException. The illustration is below.
image::remove.png[width="200']

For showAllPhones, if there is no additional numbers, only the primary number will be displayed. Illustration is below.
image::showAllPhonesWithNoAdditionalPhones.png[width="200']

If there are numbers in the uniquePhoneList, the all numbers will be displayed with an index in front of it. Illustration is below.
image::showAllPhones.png[width="200']

==== Storage
New XML element for UniquePhoneList and new XmlAdaptedPhone class with XML value Phone are created to allow the storage for unique phone list and optional phones.
==== Ui
A new label will be displaced for each person for each instance of the same fields.
